...
test_connect_and_version_output() {
  assert_matches "mariadbd  Ver [0-9]+.[0-9]+.[0-9]+-MariaDB*" "$(docker run --rm --entrypoint mariadbd philippdormann/mariadb-alpine:${IMAGE_VERSION} --version)" "MariaDB should output a version"
}
